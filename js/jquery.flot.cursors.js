...
modif label color & background 2017 by manu
*/
...
(function ($) {
    ...
    function init(plot) {
        ...
        function onMouseUp(e) {
            ...
            if (currentlySelectedCursor) {
                ...
                if (currentlySelectedCursor.dragmode.indexOf('x') !== -1) {
					plot.getPlaceholder().trigger('cursorupdatesx', [false]);
                    currentlySelectedCursor.x = mouseX;
                    currentlySelectedCursor.mousePosition.relativeX = mouseX / plot.width();
                    //currentlySelectedCursor.position.relativeX = currentlySelectedCursor.x / plot.width(); //by manu
                }
        ...
        function onMouseMove(e) {
            ...
            if (currentlySelectedCursor) {
                if (currentlySelectedCursor.dragmode.indexOf('x') !== -1) {
					//console.log("mouseOverCursorVerticalLine");
					plot.getPlaceholder().trigger('cursorupdatesx', [true]);
                    currentlySelectedCursor.x = mouseX;
                    currentlySelectedCursor.position.relativeX = currentlySelectedCursor.x / plot.width();
                    currentlySelectedCursor.mousePosition.relativeX = mouseX / plot.width();
                }
	...
	function drawTextBG(ctx, txt, font, x, y,lcolor,bcolor,align) {
		ctx.save();
		ctx.font = font;
		ctx.textBaseline = 'top';
		ctx.fillStyle = bcolor;//"rgba(255, 255, 255, 0.5)";//bcolor;// add by manu
		...
		if(align == 'left'){
			...
			ctx.fillRect(x - width , y, width, parseInt(font, 10));
		}
		ctx.fillStyle = lcolor;// add by manu
    ...
    function drawLabel(plot, ctx, cursor) {
        if (cursor.showLabel) {
			...
			drawTextBG(ctx, cursor.name, ctx.font, position.x, position.y,cursor.labelcolor,cursor.labelbackground,position.textAlign);
            ctx.textAlign = 'left';
            ctx.stroke();
        }
    }
    ...
    function mouseOverCursorManipulator(e, plot, cursor) {
		...
		var grabRadius = constants.symbolSize + cursor.mouseGrabMargin;// by manu
    ...
    function mouseOverCursorVerticalLine(e, plot, cursor) {
		...
		return (hasVerticalLine(cursor) && (mouseX > cursor.x - cursor.mouseGrabMargin) &&
            (mouseX < cursor.x + cursor.mouseGrabMargin) && (mouseY > 0) && (mouseY < plot.height()));	//by manu		
    }
    ...
    function mouseOverCursorHorizontalLine(e, plot, cursor) {
		...
		return (hasHorizontalLine(cursor) && (mouseY > cursor.y - cursor.mouseGrabMargin) &&
            (mouseY < cursor.y + cursor.mouseGrabMargin) && (mouseX > 0) && (mouseX < plot.width()));	// by manu
    }
